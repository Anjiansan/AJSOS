...
void HariMain(void)
{
    ...
    io_out8(PIC1_IMR,0xEF); //鼠标设置为许可(11011111)
    ...
    *((int *)(task_cons->tss.esp+4))=(int)sht_cons;
    task_run(task_cons,2,2);    //level=2,priority=0
        ...
        else
        {
            ...
            if(256<=i && i<=511)    //键盘数据
            {
                ...
                if(i==256+0x0E)   //退格键
                {
                    if(key_to==0)
                    {
                        if(cursor_x>8)    //用空格键把光标消去后,后移1次光标
                        {
            ...
            else if(512<=i && i<=767)   //鼠标数据
            {
                ...
                //    鼠标指针的移动
                    ...
                    sprintf(s,"(%3d,%3d)",mx,my);
                    putfont8_asc_sht(sht_back,0,0,COL8_FFFFFF,COL8_008484,s,10);//显示坐标
                    sheet_slide(sht_mouse,mx,my);
                    if((mdec.btn & 0x01)!=0)    //按下鼠标左键,移动sht_win
                    {
